...
The `MotionTransition` component applies interpolate-based animations to `transform`, `opacity`, and `filter` properties. It supports standard distinct properties like `x`, `y`, `scale`, `rotate`, as well as staggered animations for children.
